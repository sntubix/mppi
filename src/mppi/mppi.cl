...
float get_obstacle_dist(float reaction_time,
    ...
    for (uint i=0; i < nb_objects; i++) {
        ...
        float dx = obj_x - x;
        float dy = obj_y - y;
        //float obj_d = fmax(sqrt(dx * dx + dy * dy) - obj_r, 0.0f);
        ...
        if (dir_dist != -1.0f) {
            float horizon = (vel + (accel * reaction_time)) * reaction_time + vehicle_length;
            if ((dir_dist <= obj_r) && (obj_d < horizon)) {
                min_obj_d = fmin(min_obj_d, obj_d);
            }
        }

        // compute min object distance
        uint offset_w = 0;
        for (uint j=0; j < nb_waypoints; j++) {
            // don't consider objects behind vehicle
            float w_x = cos_theta * obj_x + sin_theta * obj_y - cos_theta * x - sin_theta * y;
            float w_y = -sin_theta * obj_x + cos_theta * obj_y + sin_theta * x - cos_theta * y;
            if (w_x < -obj_r) {
                continue;
            }

            float dx = obj_x - waypoints[offset_w];
            float dy = obj_y - waypoints[offset_w+1];
...
__kernel void mppi(uint N,
    ...
    const float wheel_base = params[4];
    const float max_steering = params[5];
    const float max_steering_rate = params[6];
    const float max_accel = params[7];
    const float min_accel = params[8];
    ...
    */
    ...
    for (uint i=0; i < N; i++) {
        ...
        if ((vehicle_vel < 1.0f) && (vehicle_steering < 0.07f)) {
            _max_steering_rate = 0.05f; //TODO args or remove
        }
        ...
        // collision checker
        float min_obj_d = get_obstacle_dist(reaction_time, go_around, stop_velocity,
                                            vehicle_length, vehicle_x, vehicle_y, vehicle_vel,
                                            x, y, yaw, vel, accel,
                                            waypoints, waypoints_size, waypoint_dim, offset_c_w,
                                            objects, objects_size, object_dim);

        // safe distance from obstacle
        float target_safe_dist = safe_dist_coef * vel + safe_dist_min;
        float safe_dist = fmax((target_safe_dist - min_obj_d), 0.0f);

        // compute cost
        float c_min_dist = 15.0f * min_dist * min_dist;
